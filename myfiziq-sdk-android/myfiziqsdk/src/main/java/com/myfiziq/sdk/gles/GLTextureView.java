 ...
 * SOURCE:
 *
 *      https://gist.github.com/eleventigers/9545428
 *
 *
 */
public class GLTextureView
    ...
    protected void onAttachedToWindow() {
        ...
        if (LOG_ATTACH_DETACH) {
            Timber.d("onAttachedToWindow reattach =%s", mDetached);
        }
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        // Removed as per: https://gist.github.com/eleventigers/9545428#gistcomment-1333093
        //requestRender();
    }
    ...
    private class DefaultContextFactory implements EGLContextFactory {
        ...
        public void destroyContext(EGL10 egl, EGLDisplay display,
            ...
            if (!egl.eglDestroyContext(display, context)) {
                ...
                if (LOG_THREADS) {
                    Timber.i("tid=%s", Thread.currentThread().getId());
                }
    ...
    private static class DefaultWindowSurfaceFactory implements EGLWindowSurfaceFactory {
        ...
        public EGLSurface createWindowSurface(EGL10 egl, EGLDisplay display,
            ...
            try {
                ...
                Timber.e(e, "eglCreateWindowSurface");
            }
            return result;
        }
    ...
    private static class EglHelper {
        ...
        public void start() {
            if (LOG_EGL) {
                Timber.w("start() tid=%s", Thread.currentThread().getId());
            }
            /*
             * Get an EGL instance
             */
            ...
            if (LOG_EGL) {
                Timber.w("createContext " + mEglContext + " tid=" + Thread.currentThread().getId());
            }
        ...
        public boolean createSurface() {
            if (LOG_EGL) {
                Timber.w("createSurface() tid=%s", Thread.currentThread().getId());
            }
            /*
             * Check preconditions.
             */
        ...
        public void destroySurface() {
            if (LOG_EGL) {
                Timber.w("destroySurface() tid=%s", Thread.currentThread().getId());
            }
            destroySurfaceImp();
        }
        ...
        public void finish() {
            if (LOG_EGL) {
                Timber.w("finish() tid=%s", Thread.currentThread().getId());
            }
            if (mEglContext != null) {
                GLTextureView view = mGLSurfaceViewWeakRef.get();
                if (view != null) {
        ...
        public static void throwEglException(String function, int error) {
            ...
            if (LOG_THREADS) {
                Timber.e("throwEglException tid=" + Thread.currentThread().getId() + " "
                        + message);
            }
    ...
    static class GLThread extends Thread {
            ...
            try {
                ...
                while (true) {
                    synchronized (sGLThreadManager) {
                        while (true) {
                            ...
                            if (mPaused != mRequestPaused) {
                                ...
                                if (LOG_PAUSE_RESUME) {
                                    Timber.i("mPaused is now " + mPaused + " tid=" + getId());
                                }
                            }
                            ...
                            if (mShouldReleaseEglContext) {
                                if (LOG_SURFACE) {
                                    Timber.i("releasing EGL context because asked to tid=%s", getId());
                                }
                            ...
                            if (pausing && mHaveEglSurface) {
                                if (LOG_SURFACE) {
                                    Timber.i("releasing EGL surface because paused tid=%s", getId());
                                }
                                stopEglSurfaceLocked();
                            }
                            ...
                            if (pausing && mHaveEglContext) {
                                ...
                                if (!preserveEglContextOnPause || sGLThreadManager.shouldReleaseEGLContextWhenPausing()) {
                                    stopEglContextLocked();
                                    if (LOG_SURFACE) {
                                        Timber.i("releasing EGL context because paused tid=%s", getId());
                                    }
                                }
                            }
                            ...
                            if (pausing) {
                                if (sGLThreadManager.shouldTerminateEGLWhenPausing()) {
                                    mEglHelper.finish();
                                    if (LOG_SURFACE) {
                                        Timber.i("terminating EGL because paused tid=%s", getId());
                                    }
                                }
                            }
                            ...
                            if ((! mHasSurface) && (! mWaitingForSurface)) {
                                if (LOG_SURFACE) {
                                    Timber.i("noticed surfaceView surface lost tid=%s", getId());
                                }
                                if (mHaveEglSurface) {
                                    stopEglSurfaceLocked();
                                }
                            ...
                            if (mHasSurface && mWaitingForSurface) {
                                if (LOG_SURFACE) {
                                    Timber.i("noticed surfaceView surface acquired tid=%s", getId());
                                }
                            ...
                            if (doRenderNotification) {
                                if (LOG_SURFACE) {
                                    Timber.i("sending render notification tid=%s", getId());
                                }
                            ...
                            if (readyToDraw()) {
                                ...
                                if (mHaveEglSurface) {
                                    if (mSizeChanged) {
                                        ...
                                        if (LOG_SURFACE) {
                                            Timber.i("noticing that we want render notification tid=%s", getId());
                                        }
                    ...
                    if (LOG_RENDERER_DRAW_FRAME) {
                        Timber.w("onDrawFrame tid=%s", getId());
                    }
                    {
                        GLTextureView view = mGLSurfaceViewWeakRef.get();
                        if (view != null) {
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            if (LOG_SURFACE) {
                                Timber.i("egl context lost tid=%s", getId());
                            }
                            lostEglContext = true;
                            break;
                        default:
        ...
        public void surfaceCreated() {
            synchronized(sGLThreadManager) {
                if (LOG_THREADS) {
                    Timber.i("surfaceCreated tid=%s", getId());
                }
                mHasSurface = true;
                sGLThreadManager.notifyAll();
        ...
        public void surfaceDestroyed() {
            synchronized(sGLThreadManager) {
                if (LOG_THREADS) {
                    Timber.i("surfaceDestroyed tid=%s", getId());
                }
                mHasSurface = false;
                sGLThreadManager.notifyAll();
                while((!mWaitingForSurface) && (!mExited)) {
        ...
        public void onPause() {
            synchronized (sGLThreadManager) {
                if (LOG_PAUSE_RESUME) {
                    Timber.i("onPause tid=%s", getId());
                }
                mRequestPaused = true;
                sGLThreadManager.notifyAll();
                while ((! mExited) && (! mPaused)) {
                    if (LOG_PAUSE_RESUME) {
                        Timber.i("onPause waiting for mPaused.");
                    }
                    try {
                        sGLThreadManager.wait();
        ...
        public void onResume() {
            synchronized (sGLThreadManager) {
                ...
                while ((! mExited) && mPaused && (!mRenderComplete)) {
                    if (LOG_PAUSE_RESUME) {
                        Timber.i("onResume waiting for !mPaused.");
                    }
                    try {
                        sGLThreadManager.wait();
                    } catch (InterruptedException ex) {
        ...
        public void onWindowResize(int w, int h) {
            synchronized (sGLThreadManager) {
                ...
                while (! mExited && !mPaused && !mRenderComplete
                        && ableToDraw()) {
                    if (LOG_SURFACE) {
                        Timber.i("onWindowResize waiting for render complete from tid=%s", getId());
                    }
                    try {
                        sGLThreadManager.wait();
                    } catch (InterruptedException ex) {
    ...
    static class LogWriter extends Writer {
        ...
        private void flushBuilder() {
            if (mBuilder.length() > 0) {
                Timber.d("GLTextureView %s", mBuilder);
    ...
    private static class GLThreadManager
    {
        public synchronized void threadExiting(GLThread thread) {
            if (LOG_THREADS) {
                Timber.i("exiting tid=%s", thread.getId());
            }
            thread.mExited = true;
            if (mEglOwner == thread) {
                mEglOwner = null;
...
//                }
//                if (LOG_SURFACE) {
//                    Timber.w("checkGLESVersion mGLESVersion =" +
